...
/*
You will be able to code in and run this file in the prolog interpreter 
directly. I recommend reading this assignment directly from the source file.

We will be using swipl for our prolog environment.
To load/reload this file, cd to its directory and run swipl. Then, in the 
prompt, type '[hw3].'
...
/* The following 2 problems will not be graded and are intended
...
Now, write a predicate lst_has(X,Y) that succeeds if X is a list of items and
Y is one of the items.
...
/* Problem 2 Test: */
%:- lst_has([(1,2),(1,3)], (1,3)).     % SUCCEED
%:- lst_has([3], 3).         % SUCCEED
%:- lst_has([1,2,3],4).      % FAIL
...
/* Problem 3:
 Write a predicate init(All, Blst) that succeeds if Blst is the same list as 
 All except with the last item of All removed.  The items in BLst are in the 
 same order as All.
*/
...
/* Problem 9:
   ...
   Define a predicate less_than/2 that takes two numerals in the notation that 
   we introduced in the text (that is, 0, succ(0), succ(succ(0)), and so on)
...
/* Problem 10:
   ...
   Use the add/3, from problem 0A, definition to define subtract/3.  Do not
...
/* Problem 11:  
   ...
   Convert this definition to a Prolog program. In addition provide 3 test 
   cases.  At least one test case m should have a value greater than 0.
*/
